...
#[tracing::instrument(skip(state, user), fields(user_id = %user.macro_user_id), err(Debug))]
pub async fn get_agent<S: AgentProxyService, Auth: MacroAuthorizationService>(
    ...
    let response = state
        .service
        .get_agent(user.macro_user_id, agent_id)
        .await?;
    Ok(Json(response))
}
...
pub async fn delete_agent<S: AgentProxyService, Auth: MacroAuthorizationService>(
    ...
    state
        .service
        .delete_agent(user.macro_user_id, agent_id)
        .await?;
    Ok(Json(EmptyResponse::default()))
}
...
pub async fn permanently_delete_agent<S: AgentProxyService, Auth: MacroAuthorizationService>(
    ...
    state
        .service
        .permanently_delete_agent(user.macro_user_id, agent_id)
        .await?;
    Ok(Json(EmptyResponse::default()))
}
...
pub async fn provision_runtime_connection<S: AgentProxyService, Auth: MacroAuthorizationService>(
    ...
    let url = state
        .service
        .provision_runtime_connection(user.macro_user_id, agent_id)
        .await?;
...
#[tracing::instrument(skip(state, user, message), fields(user_id = %user.macro_user_id), err(Debug))]
pub async fn post_acp<S: AgentProxyService, Auth: MacroAuthorizationService>(
    ...
    state
        .service
        .post_acp(user.macro_user_id, session_id, message)
        .await?;
