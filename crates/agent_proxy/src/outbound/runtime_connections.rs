...
use agent_runtime_protocol::outbound::jsonrpsee::ServerTransport;
use anyhow::Context;
use macro_uuid::Uuid;
...
/// is in use.
///
/// `jsonrpsee::server::ServerHandle::stop` gracefully shuts down connections
/// the server is already serving, not just future ones - so the listener
/// must outlive the connection it accepted, rather than being torn down the
/// moment `accept()` resolves. Dropping this guard (once the connection is
/// actually done) is what reclaims the listener.
pub struct ConnectionGuard(#[allow(dead_code)] jsonrpsee::server::ServerHandle);
...
impl EphemeralRuntimeConnections {
    ...
    async fn bind_in_range(&self) -> anyhow::Result<(jsonrpsee::server::Server, u16)> {
        ...
        for attempt in 0..span {
            ...
            if let Ok(server) = jsonrpsee::server::Server::builder()
...
impl RuntimeProvisioner for EphemeralRuntimeConnections {
    ...
    async fn provision(&self, session_id: Uuid) -> anyhow::Result<String> {
        ...
        let (websocket, port) = self
            .bind_in_range()
            .await
            .context("failed to bind ephemeral runtime listener")?;
        let handle = websocket.start(
            transport
                .rpc_module()
                .context("failed to register runtime rpc module")?,
        );

        let incoming = self.incoming.clone();
        tokio::spawn(async move {
            if let Some(channel) = transport.accept().await {
                let _ = incoming.send((session_id, channel, ConnectionGuard(handle)));
            }
            // No runtime ever dialed in: `handle` (never handed off above)
            // drops here, reclaiming the listener. Otherwise the guard
            // travels with the channel and reclaims it once the driven
