...
use agent_runtime_protocol::domain::connection::{CommandHandler, RuntimeConnection};
use agent_runtime_protocol::domain::schema::v0::{
    Command, CommandResult, SystemEvent, ToRuntimeMessage, ToServerMessage,
};
use agent_runtime_protocol::outbound::jsonrpsee::connect_runtime;
...
/// Log commands from the service and report them completed; the proxy does
/// not currently require any runtime commands.
struct LogCommands;

impl CommandHandler for LogCommands {
    fn handle(&self, command: Command) -> impl Future<Output = CommandResult> + Send {
        tracing::info!(?command, "received command");
        std::future::ready(CommandResult::completed())
    }
}
...
async fn main() -> Result<()> {
    ...
    let channel = connect_runtime::<_, ToServerMessage, ToRuntimeMessage>(client);
    let (runtime, acp) = RuntimeConnection::connect(channel, LogCommands);
