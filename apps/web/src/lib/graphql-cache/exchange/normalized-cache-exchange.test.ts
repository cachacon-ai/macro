...
describe('normalizedCacheExchange', () => {
  ...
  describe('mutations', () => {
    ...
    it('deletes deduplicated entity records identified by mutation refs', async () => {
      const data = {
        renameEntities: {
          results: [
            {
              entity: { type: 'DOCUMENT', entityId: 'document-1' },
              affectedEntities: [
                { type: 'PROJECT', entityId: 'project-1' },
                { type: 'DOCUMENT', entityId: 'document-1' },
              ],
            },
          ],
        },
    ...
    it('deletes requested records even when the response omits entity refs', async () => {
      const { ops } = harness(host, () => ({
        data: { renameEntities: { results: [{ success: true }] } },
      }));
    ...
    it('reads affected entity refs through GraphQL aliases', async () => {
      const data = {
        renamed: {
          results: [
            {
              changed: { kind: 'DOCUMENT', key: 'document-1' },
              touched: { kind: 'PROJECT', key: 'project-1' },
            },
          ],
        },
      };
    ...
    it('reports record deletion errors without dropping the mutation result', async () => {
      ...
      const onCacheError = vi.fn();
      const data = { renameEntities: { results: [{ success: true }] } };
