...
import { getSoupEntityById, invalidateAllSoup } from '@queries/soup/cache';
...
export function EmailProvider(props: FlowProps<{ threadID: string }>) {
  ...
  const markThreadNotDone = () => {
    ...
    if (entity && markNotDoneAction.canExecute(entity)) {
      ...
      // with the thread's): unarchive directly. The removed row can't be
      // patched back optimistically, so refetch the lists to restore it.
